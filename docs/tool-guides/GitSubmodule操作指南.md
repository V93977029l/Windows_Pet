...
本项目采用 Git Submodule 技术管理外部依赖库和参考项目，实现代码的模块化管理与复用。本指南详细说明子模块的初始化、更新、提交及日常运维流程。
...
[submodule "external/godot-cpp"]
	...
	update = checkout
	shallow = true
[submodule "external/liquid-glass-studio"]
	path = external/liquid-glass-studio
	url = https://github.com/iyinchao/liquid-glass-studio.git
```

- **branch**: godot-cpp 锁定到 master 分支，获取最新功能
...
```bash
# 1. 取消注册子模块
git submodule deinit -f external/godot-cpp

# 2. 删除子模块目录
git rm -f external/godot-cpp

# 3. 删除子模块缓存目录（可选）
rm -rf .git/modules/external/godot-cpp
```
...
2. **分支选择**: godot-cpp 使用 master 分支获取最新功能
3. **定期同步**: 定期检查上游更新，确认兼容性后同步子模块版本
...
# 预期输出示例：
#  d5cc777a89d899665fb61f1650ef0dc0cf6488c4 external/godot-cpp (heads/master)
